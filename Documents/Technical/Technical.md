...
- [3. Game Mechanics](#3-game-mechanics)
  - [3.1. Core Mechanics](##3.1-core-mechanics)
    - [3.1.1. Walking ](###3.1.1-walking)
    - [3.1.2. Jumping ](###3.1.2-jumping)
    - [3.1.3. Attacking ](###3.1.3-attacking)
    - [3.1.4. Dashing ](###3.1.4-dashing)
...
The goal of this project gives us more freedom than the previous ones: we need to create a small but functional video game that is bug-free and polished. Furthermore, we can choose the technologies to proceed with the realization of the game.
...
This is a 2D side scroller action game where you play as a samuraï named Kazuo during feudal Japan's era. The player's objective is to travel throughout a level where he must defend himself against enemies with his katana. 
...
## 2.1. Game Engine
The rendered product will be a game made with Unreal Engine, a famous and free game engine developed by Epic Games. 

Indeed, this amazing software enable creators across industries to deliver cutting-edge content, interactive experiences, and immersive virtual worlds. You can make any type of game such as platformers, FPS or side scrollers (2D or 3D). 
...
After choosing the game category, we have a variety of scene templates used as a starting point depending on the kind of game we want (FPS, Virtual reality, etc...).
...
In fact, it's the 2D paper version of another template, Side Scroller, more oriented towards 3D animation.
...
When discovering Unreal Engine while ccreating a scene, there are several tools and editors to take into account.

#### 2.2.3.1 Level Editor

By default, Unreal Engine 4 will open to this interface named the Level Editor: 

<img src="./Images/LevelEditor.png" width="500" height="500" style="text-align:center;"  />

It's the primary editor where we construct our gameplay levels.  Indeed, it provides us the core level creation feature for Unreal Editor. Its purpose it to help us create, view, and modify levels, by mainly placing, transforming, and editing the properties of Actors.

Furthermore, it shows a level within a 3D environment (even in 2D) in which we place a series of objects and geometry to define the game world we want to conceive, shown in the world Outliner. There are also details about each objects, such has the Transform properties having numeric entry fields for the X, Y and Z axes. 

Indeed, we can modify their location, their rotation, and their scale. It's also possible to change the perspective view of the scene.
...
<img src="./Images/BlueprintEditor.png" width="500" height="500" style="text-align:center;" />
...
- Event Graph: it contains a node graph that uses events and function for calling  to perform actions in response to gameplay events related to the Blueprint. Indeed, this is where we focus on the programming of our character's behavior depending on the player's input.

#### 2.2.3.3 Other Editors

And for our 2D Side Scrolling game, we also used these editors very often:

- Paper 2D Sprite Editor: it's a tool to set up and edit individual sprite assets. We can decide its dimensions, its collision settings, and also the sprite its based from.

<img src="./Images/spriteEditor.png" width="500" height="500" style="text-align:center;" />
...
<img src="./Images/flipbookEditor.png" width="500" height="500" style="text-align:center;"/>
...
- Visual Studio Code: a code editor used to write programs, but also to build and debug modern web and cloud applications. However, we mostly use it to write our documentation in markdown format.
...
- The character sprite, which uses a pixelized picture representing Kazuo, and can itself contain a box for simulating the attack's hitbox.
...
- The capsule collision, which is used to simulate collision with other objects like obstacles or enemies. Its half height is 96 pixels and his radius is 40 pixels.

Another component show the character's movement: it allows to hand the players different movement's physics.

- Walk: the character as a max ground speed of 600.0. He can step at a maximum height of 45 pixels without jumping, and a ground friction of 3.0 to affect movement control when braking. Kazuo can also walk off ledges and maintain horizontal ground velocity.
...
Regarding the boss levels: the player will face an enemy stronger than any common foes. The bosses have several HPs, and the player must find the right strategy to defeat him before he is killed or if he runs out of time. Unlike regular levels, the player will have 3 chances to defeat the boss: if he is killed while having at least one chance, he will start at the boss level again.
...
In the main menu, a tutorial level to get used to the gameplay's mechanics will be also proposed to the player.
...
With his mouse, the player has a menu of 4 different choices:
- Tutorial: allows you to train your reflexes by testing a tutorial level and get used to Kazuo's actions.
- New Game: allows you to load the game before you start playing.
...
- Up Key: allows your character to jump. If used in midair, he performs a double jump.
...
- The time: always located at the top left corner of the screen, it gives the player information about the remaining time.
Indeed, it acts as a warning timecode, telling the player how much he has left to complete the level before a game over occurs, but it also displays the actual record time of completing the game.

- The enemy counter: on the top right corner, a number is constantly displayed in some levels. It represents the number of enemies the player must kill in order to finish the level. The number decreases by 1 each time an enemy is defeated, and if it hits 0, a door located at the end of the level will disappear, allowing Kazuo to progress.

- Game Over : it is located in the middle of the screen. However, it only appears when the player is defeated or has no time left, so it displays "GAME OVER". 

# 6. Optimization

Unreal Engine provides us many features and possibility to build our game. However, the game's size became heavier the more we use countless assets, such as tiles which take a lot of place. We also need to make the game's screen resolution appealing enough to make the player motivated to play Ronin's Revenge.

Dealing with the memory used for creating our game is important. Indeed, we want to avoid having issues when pushing the game's codes and files in github. So, the game's performance must remain almost untouched when transferring data in the main branch of our project.
...
<img src="./Images/LevelOfDetails.png" width="500" height="500" style="text-align:center;"/>
